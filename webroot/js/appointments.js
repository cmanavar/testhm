$(document).ready(function () {
    ...
    $('#calendar').fullCalendar({// set header and view of calander
        ...
        eventMouseover: function (data, event, view) {  // display mousehover popup
            $('.copypopup').remove();
            ...
            if(data.selectworktodo != ""){
                ...
                if(data.worktodo != null) {
                    tooltip = tooltip.replace("##WORKTODO##", data.worktodo);   
                } else {
                    tooltip = tooltip.replace("##WORKTODO##", "");   
                }
            }
    ...
    var $select = $('#SELECT-WORK-TO-DO').selectize({// Work to do selectize dropdown
        ...
        onOptionAdd: function (value, data) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.success) {
                        $("#addworktodo").val(data.result.id);
...
//                            $('.AppDatePopup').val((date.getDate()) + '-' + (date.getMonth() + 1) + '-' + date.getFullYear());
    ...
    function cleardropdown() {
       // alert('clear')
        ...
        var $select5 = $("#SELECT-DOCTOR").selectize();
        var selectize5 = $select5[0].selectize;
        selectize5.setValue($("#defaultdr").val()); 
        
       
        $('#appointment_id').val("");
        
        //var $select6 = $('#SELECT-WORK-TO-DO').selectize();
        //var selectize6 = $select6[0].selectize;
        //selectize6.addOption({text:'Foo123',value: 'foo456'});
        //selectize6.refreshItems();
